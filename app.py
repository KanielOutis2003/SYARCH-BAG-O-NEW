...
def admin_dashboard():
    ...
    # Note: We no longer clear active sessions automatically
    # This ensures approved sessions remain in the Forms tab until checked in
...
def get_pc_status():
    ...
    try:
        ...
        pc_status = {}
        for i in range(1, 51):  # Supporting 50 PCs per lab
            pc_status[str(i)] = {
                'status': 'vacant',
                'student_id': None,
                'session_id': None,
...
def update_pc_status():
    ...
    try:
        # If setting to vacant, mark ALL potentially matching sessions as completed
        if status == 'vacant':
            # Mark all active or pending sessions for this PC as completed
            cursor.execute("""
            UPDATE sessions 
            ...
            # Also mark any session that might not have the PC number in the purpose
            cursor.execute("""
            UPDATE sessions 
            SET status = 'completed', check_out_time = NOW()
            WHERE lab_room = %s 
        ...
        elif status in ['occupied', 'reserved']:
            # If marking as occupied/reserved, create a manual admin entry
...
def add_session():
    ...
    if not lab_room or not pc_number or not date or not time_in:
    ...
    # Format the purpose with PC number
    reservation_purpose = f"{final_purpose} - PC #{pc_number}"
    ...
    try:
        ...
        # Insert the session - all PCs are available, so we don't need to check availability
        cursor.execute("""
        ...
        """, (session['user_id'], lab_room, date_time, duration, reservation_purpose))
